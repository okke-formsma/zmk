...
#define SCROLL_HORIZONTAL(encoded) (((encoded)&0xFF00) >> 8)
#define SCROLL_VERTICAL(encoded) ((encoded)&0x00FF)
...
static int on_keymap_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    int32_t x = SCROLL_HORIZONTAL(binding->param1);
    int32_t y = SCROLL_VERTICAL(binding->param1);
    zmk_hid_mouse_scroll_press(x, y);
...
static int on_keymap_binding_released(struct zmk_behavior_binding *binding,
    ...
    int32_t x = SCROLL_HORIZONTAL(binding->param1);
    int32_t y = SCROLL_VERTICAL(binding->param1);
    zmk_hid_mouse_scroll_release(x, y);
    return zmk_endpoints_send_mouse_report();
}
