...
#define DT_DRV_COMPAT zmk_behavior_okke
...
struct behavior_okke_behaviors {
...
struct behavior_okke_config {
  timer_func tapping_term_ms;
  struct behavior_okke_behaviors* behaviors;
};

struct behavior_okke_data {
  struct k_timer timer;
};

static void timer_handler(struct k_timer *timer)
{
  const struct behavior_okke_config *cfg = k_timer_user_data_get(timer);
  const struct behavior_okke_behaviors *behaviors = cfg->behaviors;
...
static int behavior_okke_init(struct device *dev)
{
  struct behavior_okke_data *data = dev->driver_data;
...
{
  struct behavior_okke_data *data = dev->driver_data;
  const struct behavior_okke_config *cfg = dev->config_info;
...
static const struct behavior_driver_api behavior_okke_driver_api = {
...
#define KP_INST(n) \
  static k_timeout_t behavior_okke_config_##n##_gettime() { return K_MSEC(DT_INST_PROP(n, tapping_term_ms)); } \
  static struct behavior_okke_behaviors behavior_okke_behaviors_##n = { \
    .tap = _TRANSFORM_ENTRY(0, n) \
    .hold = _TRANSFORM_ENTRY(1, n) \
  }; \
  static struct behavior_okke_config behavior_okke_config_##n = { \
    .behaviors = &behavior_okke_behaviors_##n, \
    .tapping_term_ms = &behavior_okke_config_##n##_gettime, \
  }; \
  static struct behavior_okke_data behavior_okke_data_##n; \
  DEVICE_AND_API_INIT(behavior_okke_##n, DT_INST_LABEL(n), behavior_okke_init, \
                      &behavior_okke_data_##n, \
                      &behavior_okke_config_##n, \
                      APPLICATION, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, \
                      &behavior_okke_driver_api);
